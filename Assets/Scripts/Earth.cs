...
public class Earth: Planeta
{
    // Override InitializeOrbit method from PlanetGravity
    protected override void InitializeOrbit()
    {
        gravitationalConstant = 1f;  // Override default value if needed
        planetMass = 200f;            // Speed and rate of attraction (planet mass)
    ...
    void LateUpdate(){
        float yRotation = -10 * Time.deltaTime;
